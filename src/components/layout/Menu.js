...
class InuMenu extends React.Component {
    ...
    renderItem(item){
        if (item.to&&item.icon)return <Link to={{...this.props.location,pathname:item.to}}><Icon type={item.icon} />{item.title}</Link>
        if (item.to)return <Link to={{...this.props.location,pathname:item.to}}>{item.title}</Link>
    ...
    renderMenu(menu,i){
        ...
        if (menu.group) return <MenuItemGroup key={this.state.count++} title={menu.group}>{this.renderMenu(menu.menu)}</MenuItemGroup>
