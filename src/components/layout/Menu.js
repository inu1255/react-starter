...
class InuMenu extends React.Component {
  ...
  selectedKeys(menu, i) {
    ...
    if (menu.to !== undefined && this.props.router.isActive({
          pathname: menu.to
        })) {
      return i
    }
  ...
  renderItem(item) {
    if (item.to && item.icon) return <Link to={ { ...this.props.location, pathname: item.to } }>
                                     <Icon type={ item.icon } />
                                     { item.title }
                                     </Link>
    if (item.to) return <Link to={ { ...this.props.location, pathname: item.to } }>
                        { item.title }
                        </Link>
